...
<body class="flex flex-col min-h-[100vh]">
  <header class="bg-slate-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
      <div class="py-6">
        <a href="/tasks" class="text-white text-xl">Todoアプリ</a>
      </div>
    </div>
  </header>
  ...
  <main class="grow">
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
      <div class="py-[100px]">
        <p class="text-2xl font-bold text-center">今日は何をしますか？</p>
        <form action="/tasks" method="post" class="mt-10">
          @csrf
          <div class="flex flex-col items-center">
            <label class="w-full max-w-3xl mx-auto">
              <input type="text" name="task_name" placeholder="テスト勉強する..." class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-4 pl-4 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm"/>
              @error('task_name')
                <div class="mt-3">
              ...
              <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                <table class="min-w-full divide-y divide-gray-300">
                  <thead class="bg-gray-50">
                    <tr>
                      <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900">未完了タスク</th>
                                ...
                                <td class="p-0 text-right text-sm font-medium">
                                    <div class="flex justify-end">
                                        ...
                                        <div>
                                            ...
                                            @csrf
                                            @method('PUT')<!--formのactionには、postかgetしか指定できないため、ここで任意のactionを指定/.-->
                                            <!--updateメソッドを編集でも使用しており、区別するため下記を書く/.-->
                                        ...
                                        <div>
                                            <form onsubmit="return deleteTask()" action="/tasks/{{ $item->id }}" method="post" class="inline-block text-gray-500 font-medium" role="menuitem" tabindex="-1">
                                                @csrf
                                                @method('DELETE')<!--formのactionには、postかgetしか指定できないため、ここで任意のactionを指定/.-->
